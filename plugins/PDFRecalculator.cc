...
private:
  ...
  edm::EDGetTokenT<LHEEventProduct> lheEventInfoTag_;
  bool recalculatePDFs_, recalculateScales_;
  unsigned nQCD_, nEM_;
...
PDFRecalculator::PDFRecalculator(const edm::ParameterSet& iConfig) :
  ...
  genProductToken_(consumes<GenEventInfoProduct>(iConfig.getParameter<edm::InputTag>("genEventInfoTag"))), //generator
  lheEventInfoTag_(consumes<LHEEventProduct>(iConfig.getParameter<edm::InputTag>("lheEventInfoTag"))), //externalLHEProducer
  ...
  pdfSetName_(iConfig.getParameter<std::string>("pdfSetName"))
{
  ...
  produces<std::vector<float>>("LHEPdfWeight");
  produces<std::vector<float>>("LHEScaleWeight");
}
...
void PDFRecalculator::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  iEvent.getByToken(lheEventInfoTag_, lhe_info);
  ...
  if(recalculatePDFs_){
    ...
    for (unsigned int i=0; i<nweights; ++i) {
      ...
      if(debug_) {
        edm::LogInfo("PDFRecalculator") << "PDFRecalculator: index = " << i << ", x1 = " << x1 << ", x2 = " << x2
                                  << ", Q = " << Q << ", id1 = " << id1 << ", id2 = " << id2
  ...
  if(recalculateScales_){
    ...
    if(debug_)
      edm::LogInfo("PDFRecalculator") << "PDFRecalculator: x1 = " << x1 << ", x2 = " << x2 << ", Q = " << Q
    ...
    if(debug_)
        edm::LogInfo("PDFRecalculator") << "PDFRecalculator: Q = " << Q << ", Q2 = " << Q2 << ", alpEM = " << alpEM
                                    << ", alpEMup = " << alpEMup << ", alpEMdn = " << alpEMdn << ", alpS = " << alpS
                                    << ", alpSup = " << alpSup << ", alpSdn = " << alpSdn << std::endl;
  ...
  iEvent.put(std::move(pdfweights),"LHEPdfWeight");
  iEvent.put(std::move(scaleweights),"LHEScaleWeight");
